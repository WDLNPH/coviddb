                            ...
                            <FieldArray name="groupZones">
                                {({ push, remove }) => (
                                    <div className="flex flex-col items-center relative">
                                        ...
                                        <div
                                            className="shadow bg-white z-40 w-full lef-0 rounded max-h-select overflow-y-auto svelte-5uyqqj">
                                            <div className="flex flex-col w-full">
                                                {groupZones.filter(gz => gz.name.toLowerCase().indexOf(filter.toLowerCase()) !== -1
                                                    && !values.groupZones.includes(gz.group_id))
                                                    .map(gz => <Option onClick={() => push(gz.group_id)} name={gz.name}/>)}
                                            </div>
                                        </div>
                                    </div>
                                )}
